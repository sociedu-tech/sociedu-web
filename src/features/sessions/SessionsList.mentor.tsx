...
export function SessionsListMentor() {
  const rows = [
    { id: 's1', title: 'Review đồ án — tuần 3', when: '20/04/2026 19:00', mentee: 'Trần B', status: 'Sắp diễn ra' },
  ];

  return (
    <div className="space-y-6">
      <p className="text-[15px] leading-relaxed text-gray">
        Buổi học bạn đã nhận / cần xác nhận.
      </p>
          ...
          <tbody className="text-dark">
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-border last:border-0">
                ...
                <td className="hidden px-4 py-3 text-gray md:table-cell">{row.mentee}</td>
